...
rm -f /home/549/jm9807/align.input_oryx.txt

awk 'NR>1' /home/549/jm9807/samples.config_oryx.txt | while read LINE
do 
	...
	fqpairs=$(ls /scratch/mo73/align_redo/*${sample}*fq.gz | sed  's/_R1.*\|_R2.*//' | uniq)
	fqpairs=($fqpairs)
	
	#Get the flowcell and lane info from the original pairs:	
	for ((i=0; i<${#fqpairs[@]}; i++))
	do
		flowcell=$(zcat ${fqpairs[i]}_R1.fq.gz  | head -1 | cut -d ':' -f 3)
		lane=$(zcat ${fqpairs[i]}_R1.fq.gz | head -1 | cut -d ':' -f 4)
		
		#Print each of the split chunks with flowcell and lane info to inputs file:
		set=$(basename ${fqpairs[i]})
		splitpairs=$(ls /scratch/mo73/align_redo/*${set}* | sed  's/_R1.*\|_R2.*//' | uniq)
		splitpairs=($splitpairs)
		
		for ((c=0; c<${#splitpairs[@]}; c++))
		do
			printf "${splitpairs[c]},${sample},${centre},${lib},${platform},${flowcell},${lane}\n" >> /home/549/jm9807/align.input_oryx.txt
		done			
	done			
done	

tasks=`wc -l < /home/549/jm9807/align.input_oryx.txt`
