...
gl_vcf <- gl.read.vcf(paste0(chrom_dir, chrom, ".vcf"))
# filtering
...
gl_vcf <- gl.filter.monomorphs(gl_vcf)
...
gl_vcf_ox <- gl.read.vcf(paste0(chrom_dir, chrom, "_oxford.vcf"))
# filtering
...
gl_vcf_ox <-
  gl.drop.loc(gl_vcf_ox, loc.list = locNames(gl_vcf_ox)[loc_unmap])
...
gl_vcf_ox <-
  gl.drop.loc(gl_vcf_ox, loc.list = locNames(gl_vcf_ox)[loc_chr])

########################################################################
################### loading DArT samples ###############################
########################################################################
gl_vcf_dart <- gl.load(paste0(chrom_dir, chrom, "_dart.rds"))
...
gl_vcf_dart <- gl.blast(gl_vcf_dart, ref_genome = ref_gen_file_unzip)
pos <-
...
x_dart <- gl.keep.ind(gl_vcf_dart_jenna, ind.list = indNames(gl_vcf))
x_dart <- gl.filter.callrate(x_dart, threshold = 1)
x_dart <- gl.filter.monomorphs(x_dart)

x_jenna <- gl.filter.callrate(gl_vcf, threshold = 1)

t2 <- which(x_dart$position %in% x_jenna$position)
t3 <- which(x_jenna$position %in% x_dart$position)

x_dart <- gl.keep.loc(x_dart, loc.list = locNames(x_dart)[t2])
x_dart <- x_dart[, order(x_dart$position)]
x_dart <- x_dart[order(indNames(x_dart)), ]

x_jenna <- gl.keep.loc(x_jenna, loc.list = locNames(x_jenna)[t3])
...
incorrect_percentage <-
  (sum(colSums(s1 != s2)) / (nInd(x_jenna) * nLoc(x_jenna))) * 100
